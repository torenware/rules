...
class UserHasEntityFieldAccess extends RulesConditionBase {
  ...
  public function evaluate() {
    ...
    $entity_access_controller = $this->entityManager->getAccessController($entity->getEntityTypeId());
    $lang_code = Language::LANGCODE_DEFAULT;

    //"Callers are responsible for ensuring that entity access is also respected":
    $has_entity_access = $entity_access_controller->access($entity, $op, $lang_code, $account);
    if (!$has_entity_access) {
      return FALSE;
    }

    $items = $entity->$field;
    return $entity_access_controller->fieldAccess($op, $field_definition, $account, $items);
  }
}
